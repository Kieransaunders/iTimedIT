...
export function useEntries(projectId?: Id<"projects">): UseEntriesReturn {
  ...
  const createManualEntryMutation = useMutation(api.timer.createManualEntry);
  const editEntryMutation = useMutation(
    currentWorkspace === "personal" ? api.personalEntries.editPersonal : api.entries.edit
  );
  const deleteEntryMutation = useMutation(
    currentWorkspace === "personal" ? api.personalEntries.deletePersonalEntry : api.entries.deleteEntry
  );
